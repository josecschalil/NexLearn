...
const AdminExamPage = () => {
  ...
  const handleSubmit = async () => {
    const examData = {
      ...
      subject: selectedExamType === "fullsubject" ? newExam.subject : null,
      chapter: null,
      questions: []
    };
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-6 shadow-md rounded-lg mb-6">
        ...
        <button onClick={handleSubmit} className="bg-green-500 text-white px-4 py-2 rounded mt-4">{editExamId ? "Update Exam" : "Submit"}</button>
      </div>
          ...
          <tbody>
            {exams.map((exam) => (
              <tr key={exam.exam_id} className="border-b">
                ...
                <td className="p-4">{exam.time} min</td>
                <td className="p-4">{exam.difficulty}</td>
                <td className="p-4">
                  <button onClick={() => handleEdit(exam)} className="bg-yellow-500 text-white px-3 py-1 rounded mr-2">Edit</button>
                  <button onClick={() => handleDelete(exam.exam_id)} className="bg-red-500 text-white px-3 py-1 rounded">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
